...
var MINIMAL_LAMBDA = 'exports.handler=function(e,c){c.done(null,{})}';
...
function newCreateTableAction(name, table) {
  var params;
  if (table.fullSchema) {
    params = table.fullSchema;  // Caller specified full parameters
  } else {
    var attributes = {};
    var keySchema = [];
...
function newCreateLambdaAction(name, role, lambda) {
  var zip = new JSZip();
  zip.file('lambdafai-entry-point.js', MINIMAL_LAMBDA);
