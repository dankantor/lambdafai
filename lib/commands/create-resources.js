...
function waitingCallback(seconds, next) {
  if (seconds > 0) {
    return function(err, data) {
      if (!err) {
        console.log('Waiting: ' + seconds + ' sec');
        setTimeout(function() { next(err, data); }, seconds * 1000);
      } else {
        next(err);
      }
    };
...
function applyActions(actions, done) {
  ...
  async.eachSeries(actions, function(action, next) {
    ...
    method(action.params, waitingCallback(action.wait, next));
  }, done);
}
