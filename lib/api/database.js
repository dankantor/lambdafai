 ...
 *   RangeKey: the range key for items to list
 ...
 *   IndexName: The name of an index to query. This index can be any local secondary index or 
 *     global secondary index on the table.
 *   FullResult: returns the full result from DynamoDB, not just the items. Default is false
 ...
 *   ProjectionExpression: An array of attributes that should be returned. Note: the hash key must
 *     be included in this list or an error will be thrown
 *
 */
Table.prototype.list = function(params, callback) {
  ...
  if (params.RangeKey) {
    expression += ' AND #rangeKey=:rangeKey';
    attributeNames['#rangeKey'] = keyName(params.RangeKey);
    attributeValues[':rangeKey'] = keyValue(params.RangeKey);
  } else if (params.MinRangeKey && params.MaxRangeKey) {
  ...
  if (params.ProjectionExpression) {
    params.ProjectionExpression.forEach((attribute, i) => {
      var placeholder = `pe${i}`;
  ...
  if (params.IndexName) {
    ...
    delete queryParams.ConsistentRead;
  }
  ...
  if (projectionExpression.length > 0) {
    queryParams.ProjectionExpression = projectionExpression.join(',');
  }
  this.dynamo.query(queryParams, function(err, result) {
    if (params.FullResult === true){
 ...
 *   ProjectionExpression: An array of attributes that should be returned.
 */
Table.prototype.get = function(params, callback) {
  ...
  this.dynamo.get(queryParams, function(err, result) {
    if (err) {
      callback(err);
...
Table.prototype.batchGet = function(params, callback) {
  ...
  if (params.ProjectionExpression) {
    getParams.RequestItems[tableName].ProjectionExpression = [];
    getParams.RequestItems[tableName].ExpressionAttributeNames = {};
    params.ProjectionExpression.forEach((attribute, i) => {
      var placeholder = `pe${i}`;
      getParams.RequestItems[tableName].ProjectionExpression.push(`#${placeholder}`);
      getParams.RequestItems[tableName].ExpressionAttributeNames[`#${placeholder}`] = attribute;
    });
  }
 ...
 *   ShouldInsert: boolean if item should be inserted if it doesn't already exist in db
 */
Table.prototype.patch = function(params, callback) {
  ...
  if (params.ShouldInsert === true) {
    if (names.indexOf('createdAt') === -1) {
      expression += ', createdAt=:createdAt';
      attributeValues[':createdAt'] = new Date().getTime();
    }
  }

  // Add to the expression for each of the updates:
  for (var i = 0; i < names.length; i++) {
    if (updates[names[i]] !== undefined && updates[names[i]] !== null) {
      var placeholder = 'x' + i;
  ...
  if (params.ShouldInsert !== true) {
    updateObj.ConditionExpression = 'attribute_exists(createdAt)';
  }

  this.dynamo.update(updateObj, function(err, result) {
    if (err && err.name == 'ConditionalCheckFailedException') {
      err = errors.notFound();
    }
