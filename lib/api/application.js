 ...
 * @property {boolean} useACL - if true, generate an access control list table (default: false)
 */
...
Application.prototype._handleRequest = function(req, callback) {
  ...
  async.eachSeries(this.handlers, function(handler, next) {
    ...
    try {
      ...
      res.done(errors.serverError(e), null);
    }
  }, function() {
    if (!res.isDone) {
      res.body = null;
      res.error = errors.notFound();
      sendResponse(req, res, callback); // Nobody handled the request.
    } else {
      ...
      // if it's an API Gateway event, wrap it in a response with statusCode, headers and body
      
      sendResponse(req, res, callback);
    }
  });
};

function sendResponse(req, res, callback) {
  var response;
  if (req.apiGateway === true) {
    response = {
