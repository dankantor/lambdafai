 ...
 */
Lambda.prototype.get = function(path, handler, authorizer) {
  this._addHandler({type: 'route', method: 'GET', path: path, handler: handler });
  if (authorizer) {
    this._addHandler({type: 'route', method: 'APIGATEWAYAUTHORIZEREVENT', path: '/GET' + path, handler: authorizer });
  }
  return this;
};
 ...
 * @param handler {function} function with signature `function(req, res)` for handling the request
 */
Lambda.prototype.post = function(path, handler, authorizer) {
  this._addHandler({type: 'route', method: 'POST', path: path, handler: handler });
  if (authorizer) {
    this._addHandler({type: 'route', method: 'APIGATEWAYAUTHORIZEREVENT', path: '/POST' + path, handler: authorizer });
  }
  return this;
};
 ...
 * @param handler {function} function with signature `function(req, res)` for handling the request
 */
Lambda.prototype.put = function(path, handler, authorizer) {
  this._addHandler({type: 'route', method: 'PUT', path: path, handler: handler });
  if (authorizer) {
    this._addHandler({type: 'route', method: 'APIGATEWAYAUTHORIZEREVENT', path: '/PUT' + path, handler: authorizer });
  }
  return this;
};
 ...
 * @param handler {function} function with signature `function(req, res)` for handling the request
 */
Lambda.prototype.delete = function(path, handler, authorizer) {
  this._addHandler({type: 'route', method: 'DELETE', path: path, handler: handler });
  if (authorizer) {
    this._addHandler({type: 'route', method: 'APIGATEWAYAUTHORIZEREVENT', path: '/DELETE' + path, handler: authorizer });
  }
  return this;
};
 ...
 * @param handler {function} function with signature `function(req, res)` for handling the request
 */
Lambda.prototype.options = function(path, handler, authorizer) {
  this._addHandler({type: 'route', method: 'OPTIONS', path: path, handler: handler });
  if (authorizer) {
    this._addHandler({type: 'route', method: 'APIGATEWAYAUTHORIZEREVENT', path: '/OPTIONS' + path, handler: authorizer });
  }
  return this;
};
