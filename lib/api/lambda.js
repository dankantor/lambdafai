...
/**
 * Routes CODE PIPELINE requests for a given rule to the handler. 
 * You must set Code Pipeline resources manually through console or API
 * You must set 'User parameters' to the path and stage  (eg. {"path": "hello", "stage": "a"})
 * AWS Docs - http://docs.aws.amazon.com/codepipeline/latest/userguide/how-to-lambda-integration.html
 * Rule names will be the method and path, e.g. .codePipeline('/hello', fn)
 * @param path {string} the path to register the handler at
 * @param handler {function} function with signature `function(req, res)` for handling the request
 */
Lambda.prototype.codePipeline = function(path, handler) {
  this._addHandler({type: 'route', method: 'CODEPIPELINE', path: path, handler: handler });
  return this;
};
