 ...
 * Lambda must be invoked with this object shape - {"method": "Invoke", "path": "/hello", "body": {..}}
 * AWS Docs - http://docs.aws.amazon.com/lambda/latest/dg/API_Invoke.html
 * @param path {string} the path to register the handler at
 * @param handler {function} function with signature `function(req, res)` for handling the request
 */
Lambda.prototype.invoke = function(path, handler) {
  this._addHandler({type: 'route', method: 'INVOKE', path: path, handler: handler });
  return this;
};
