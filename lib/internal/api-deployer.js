...
var HTTP_STATUS_CODES = [ '200', '301', '302', '400', '401', '403', '404', '409', '500', '503' ];

// Headers we support in integration response:
var INTEGRATION_RESPONSE_PARAMETERS = {
  '200': {
    'method.response.header.Location': 'integration.response.body.header.Location',
    'method.response.header.Content-Type': 'integration.response.body.header.Content-Type',
    ...
    'method.response.header.Access-Control-Max-Age': 'integration.response.body.header.Access-Control-Max-Age',
    'method.response.header.Cache-Control': 'integration.response.body.header.Cache-Control'
  },
  '301': {
...
// Headers we support in method response:
var METHOD_RESPONSE_PARAMETERS = {
  '200': {
...
// Template for integration response. If the response content-type header is set to 'text/html', we need to
// strip out the quotes. If it is JSON, we send as is
var RESPONSE_TEMPLATES = `#set($contentType = $input.json('$.header.Content-Type'))
...
function selectorForStatus(status) {
  if (status === '200') {
    ...
    // Catch various Lambda errors as 500s:
    return ('(HTTP 500|Process exited|Task timed out).*');
  } else {
    return 'HTTP ' + status + '.*';
  }
}
...
ApiDeployer.prototype._createMethod = function(resourceID, method, lambdaName, callback) {
  ...
  async.waterfall([
    ...
    function createResponses(next) {
      console.log('    - Adding responses: ' + HTTP_STATUS_CODES.join(', '));
      async.mapSeries(HTTP_STATUS_CODES, function(status, cb) {
        self.gateway.putMethodResponse({
          ...
          httpMethod: method,
          statusCode: status,
          responseParameters: METHOD_RESPONSE_PARAMETERS[status]
        }, cb);
      }, callbacks.stripData(next));
    },
    ...
    function createIntegrationResponses(next) {
      console.log('    - Adding integration responses: ' + HTTP_STATUS_CODES.join(', '));
      async.mapSeries(HTTP_STATUS_CODES, function(status, cb) {
        self.gateway.putIntegrationResponse({
          ...
          responseParameters: INTEGRATION_RESPONSE_PARAMETERS[status],
          responseTemplates: {
            'application/json': RESPONSE_TEMPLATES
          }
        }, cb);
      }, callbacks.stripData(next));
    },
