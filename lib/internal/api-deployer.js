...
ApiDeployer.prototype._getOldResourceIDs = function(callback) {
  var self = this;
  var req = self.gateway.getResources({
  ...
  req.send(function(err, data) {
    if (!err) {
      ...
      // only return resources that were created by lambdafai so we don't delete other resources
      self.oldTopLevelResourceIDs = data.items.filter(function(res) {
        var isLambdafai = false;
        for (var r in res.resourceMethods) {
          var resourceMethod = res.resourceMethods[r];
          if (resourceMethod.methodIntegration.type === 'AWS' && 
          resourceMethod.methodIntegration.uri.indexOf('${stageVariables.stage}' !== -1)) {
            isLambdafai = true;
          }
        }
        return (res.parentId == self.rootResourceID) && (isLambdafai === true);
      }).map(function(res) {
        return res.id;
      });
