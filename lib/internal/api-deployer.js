...
ApiDeployer.prototype._createMethod = function(resourceID, method, lambdaName, options, callback) {
  ...
  async.waterfall([
    ...
    function checkCors(next) {
      if (options.cors === true) {
        self._createCorsMethod(resourceID, method, lambdaName, options, callback);
      } else {
        next();
      }
    }
...
ApiDeployer.prototype._createCorsMethod = function(resourceID, method, lambdaName, options, callback) {
  var self = this;
  var region = aws.config.region;
  var uri = 'arn:aws:apigateway:' + region + ':lambda:path/2015-03-31/functions/' +
            'arn:aws:lambda:' + region + ':' + self.accountNumber + ':function:' + lambdaName +
  ...
  async.waterfall([
    function createMethod(next) {
      console.log('  - Creating CORS method OPTIONS');
      self.gateway.putMethod({
        restApiId: self.apiID,
        resourceId: resourceID,
        httpMethod: 'OPTIONS',
    ...
    function createResponses(next) {
      console.log('    - Adding CORS method responses: 200');
    ...
    function createIntegration(next) {
      console.log('    - Creating CORS integration: ' + lambdaName);
      var params = {
        restApiId: self.apiID,
        resourceId: resourceID,
        httpMethod: 'OPTIONS',
    ...
    function createIntegrationResponses(next) {
      console.log('    - Adding CORS integration responses: 200');
    ...
    function updateResponses(next) {
      console.log('    - Updating CORS method responses: 200');
    ...
    function updateIntegrationResponses(next) {
      console.log('    - Updating CORS integration responses: 200');
