  ...
  async.waterfall([
    ...
    function updateIntegrationResponses(next) {
      if (method === 'OPTIONS') {
        ...
        const origin = options['Access-Control-Allow-Origin'] || '*';
        const headers = options['Access-Control-Allow-Headers'] || 
          'Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token';
        const methods = options['Access-Control-Allow-Methods'] || 'GET,PUT,POST,DELETE,OPTIONS';
        const age = options['Access-Control-Max-Age'] || '259200';
