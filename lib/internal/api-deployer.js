...
ApiDeployer.prototype.promote = function(fromStage, toStage, callback) {
  var self = this;
  async.waterfall([
    function(next) { self._lookUpAppID(next); },
    function(next) { self._lookUpUser(next); },
    function(next) { self._getStage(fromStage, next); },
    function(stage, next) { self._writeStage(toStage, stage.deploymentId, next); },
    function(next) { self._showSuccessMessage(toStage, next); },
  ], stripData(callback));
};
...
ApiDeployer.prototype._createResources = function(callback) {
  this._createNode(this.newResourceTree, null, stripData(callback));
};
...
ApiDeployer.prototype._getStage = function(stage, callback) {
  console.log('Getting stage: ' + stage);
  this.gateway.getStage({ restApiId: this.apiID, stageName: stage }, callback);
};

ApiDeployer.prototype._writeStage = function(stage, deploymentID, callback) {
  console.log('Deploying deployment ' + deploymentID + ' to stage ' + stage);
  var self = this;
