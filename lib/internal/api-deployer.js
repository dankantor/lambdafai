...
ApiDeployer.prototype._lookUpUser = function(callback) {
  ...
  async.waterfall([
    function getRole(next) {
      self.iam.getRole(function(err, data) {
        console.log('get role');
        console.log(data);
        if (!err) {
          self.username = 'Role::' + data.Role.RoleName;
          var arnParts = data.Role.Arn.split(':');
          ...
          next(null, false, callback);
        }
      });
    },
    function getUser(getRoleSuccess, next) {
      if (getRoleSuccess === false) {
