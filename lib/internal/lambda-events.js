...
function convertCodePipelineEvent(app, event, context) {
  var userParameters = JSON.parse(
    event['CodePipeline.job'].data.actionConfiguration.configuration.UserParameters
  );
  return {
    method: 'CODEPIPELINE',
    stage: userParameters.stage,
    path: '/' + userParameters.path,
    body: event
  };
}
...
module.exports = {
  ...
  standardizeEvent: function(app, event, context) {
    ...
    } else if (isCodePipelineEvent(event)) {
      return convertCodePipelineEvent(app, event, context);
    }
  }
};
