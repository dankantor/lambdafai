...
function isCloudWatchLogsEvent(event) {
  return !!event.awslogs &&
    !!event.awslogs.data;
}

function convertCloudWatchLogsEvent(app, event, context) {
  var buf = new Buffer(event.awslogs.data, 'base64');
  var payload = zlib.gunzipSync(buf).toString('ascii');
  var obj = JSON.parse(payload);
  var path = obj.subscriptionFilters[0];
  return {
...
function isKinesisFirehoseEvent(event) {
  return !!event.invocationId &&
    !!event.deliveryStreamArn && 
    !!event.records;
}

function convertKinesisFirehoseEvent(app, event, context) {
  var parts = event.deliveryStreamArn.split(':');
  var slashParts = [];
  var dashParts = [];
  if (parts.length >= 6) {
    slashParts = parts[5].split('/'); 
  }
  var path = slashParts[1];
  return {
...
function isLambdaInvokeEvent(event) {
  return !!event.method &&
    !!event.path &&
    event.method === 'lambdaInvoke';
}

function convertLambdaInvokeEvent(app, event, context) {
  return {
    method: 'LAMBDAINVOKE',
    stage: stageFromLambdaArn(context.invokedFunctionArn),
    path: event.path,
...
function convertSqsEvent(app, event, context) {
  var items = event.Records.map(record => {
    return record.body;
  });
  var record = event.Records[0];
  var parts = record.eventSourceARN.split(':');
...
module.exports = {
  ...
  standardizeEvent: function(app, event, context) {
    if (isApiGatewayEvent(event)) {
      ...
      return convertDynamoDbEvent(app, event, context);
    } else if (isCloudWatchLogsEvent(event)) {
      return convertCloudWatchLogsEvent(app, event, context);
    } else if (isKinesisFirehoseEvent(event)) {
      return convertKinesisFirehoseEvent(app, event, context);
    } else if (isLambdaInvokeEvent(event)) {
      return convertLambdaInvokeEvent(app, event, context);
    } else if (isSqsEvent(event)) {
      return convertSqsEvent(app, event, context);
    }
  }
};
