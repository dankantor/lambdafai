...
const serveDynamic = (pathname, request, response) => {
  try {
    ...
    if (route) {
      ...
      if (args.staticfallback) {
        serveStatic(`${process.cwd()}/backend/static/${args.staticfallback}`, request, response);
      } else {
...
const requestHandler = (request, response) => {
  try {
    ...
    let filePath = `${process.cwd()}/backend/static${pathname}`;
    fs.exists(filePath, exists => {
      if (exists === true) {
        serveStatic(filePath, request, response);
      } else {
        serveDynamic(pathname, request, response);
      }
    });
  } catch (err) {
    response.statusCode = 500;
